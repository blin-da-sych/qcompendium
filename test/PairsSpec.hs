...
spec = do
  describe "Basis instance for (a, b)" $ do
    it "form a Cartesian product of basis for Bool" $
      basis `shouldBe`
      [(False, False), (False, True), (True, False), (True, True)]
  describe "Tensor product (&*) for quantum vectors" $ do
    it "compute the tensor product of two quantum vectors (Bool, Bool)" $
    ...
    it "compute the tensor product of quantum vectors with different amplitudes" $
    ...
    it "compute the tensor product of quantum vectors for Move and Colour types" $
  ...
  describe "Quantum vector construction with (&*)" $ do
    ...
    it
      "should throw an error while computing the tensor product of a not normalized quantum vectors" $ do
