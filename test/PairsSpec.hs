...
spec = do
  describe "Basis instance for (a, b)" $ do
    it "forms a Cartesian product of basis for Bool" $
      basis `shouldBe`
      [(False, False), (False, True), (True, False), (True, True)]
  describe "Tensor product (&*) for quantum vectors" $ do
    it "computes the tensor product of two quantum vectors (Bool, Bool)" $
    ...
    it
      "computes the tensor product of quantum vectors with different amplitudes" $
    ...
    it
      "computes the tensor product of quantum vectors for Move and Colour types" $
  ...
  describe "Quantum vector construction with (&*)" $ do
    ...
    it
      "throws an error while computing the tensor product of not normalized quantum vectors" $ do
      let qa = qVector' [(False, 0.5), (True, 0.5)] -- Not normalized
          qb = qVector' [(False, 0.6), (True, 0.4)] -- Not normalized
