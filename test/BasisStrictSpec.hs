...
module BasisStrictSpec
  ( spec
  ) where

import           BasisStrict       (Colour (Blue, Red, Yellow),
...
spec = do
  ...
  describe "Probability Retrieval (amplitude)" $ do
    it "retrieves probability amplitude for the Bool type" $
      property $ \b -> do
        let vector = qVector [(False, 1), (True, 0)]
        amplitude vector b `shouldBe` bool 1 0 b
    it "retrieves probability amplitude for the Move type" $
      property $ \m -> do
        let vector = qVector [(Vertical, 0), (Horizontal, 1)]
        amplitude vector m `shouldBe` bool 0 1 (m == Horizontal)
    it "retrieves probability amplitude for the Colour type" $
      property $ \c -> do
