...
spec = do
  describe "Quantum NOT Operation (qNot)" $ do
    it "flips the amplitudes of a quantum vector of type Bool" $
      property $ \b -> do
        let vector = qVector' [(False, 1), (True, 0)]
        let result = qNot vector
        amplitude result b `shouldBe` bool 0 1 b
    it "throws an error for non-normalized vector" $ do
  ...
  describe "Quantum Operator Application (qApp') - normalized version" $ do
    ...
    it "applies a non-trivial quantum operator" $ do
      let vector = qVector' [(False, 1), (True, 0)]
          operator =
            qop
              [ ((False, False), 0.5)
  ...
  describe "Quantum Operator Construction (qop)" $ do
    it "constructs a quantum operator from a list of basis-pair amplitudes" $
  ...
  describe "Toffoli Operation (toffoli)" $ do
    it "flips the target qubit if both control qubits are True" $
    ...
    it "leaves the target qubit unchanged if at least one control qubit is False" $
  ...
  describe "qLift" $ do
    it "lifts the classical NOT function for Bool to a quantum operator" $ do
      let notOp = qLift not
    ...
    it "lifts a constant function to a quantum operator" $ do
